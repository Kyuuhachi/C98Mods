...
@ASMer class Packets extends NetworkManager {
	...
	@Override protected void channelRead0(ChannelHandlerContext p_channelRead0_1_, Packet p_channelRead0_2_) {
		if(!(getNetHandler() instanceof NetHandlerPlayClient) && !(getNetHandler() instanceof NetHandlerPlayServer)) super.channelRead0(p_channelRead0_1_, p_channelRead0_2_);
			...
			for(PacketHook hook : HookImpl.packetHooks) {
				if(dir == EnumPacketDirection.CLIENTBOUND) block |= hook.packetFromServer(p_channelRead0_2_, Minecraft.getMinecraft().thePlayer);
				if(dir == EnumPacketDirection.SERVERBOUND) block |= hook.packetFromClient(p_channelRead0_2_, ((NetHandlerPlayServer)getNetHandler()).playerEntity);
			}
	...
	@Override public void dispatchPacket(Packet inPacket, GenericFutureListener[] futureListeners) {
		if(!(getNetHandler() instanceof NetHandlerPlayClient) && !(getNetHandler() instanceof NetHandlerPlayServer)) super.dispatchPacket(inPacket, futureListeners);
			...
			boolean block = false;
			for(PacketHook hook : HookImpl.packetHooks) {
				if(dir == EnumPacketDirection.CLIENTBOUND) block |= hook.packetToServer(inPacket, Minecraft.getMinecraft().thePlayer);
				if(dir == EnumPacketDirection.SERVERBOUND) block |= hook.packetToClient(inPacket, ((NetHandlerPlayServer)getNetHandler()).playerEntity);
			}
