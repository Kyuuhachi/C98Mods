...
public class SSLSocketChannel2 implements ByteChannel, WrappedByteChannel {
	...
	public SSLSocketChannel2(SocketChannel channel, SSLEngine sslEngine, ExecutorService exec, SelectionKey key) throws IOException {
		if(channel == null || sslEngine == null || exec == null) throw new IllegalArgumentException("parameter must not be null");
	...
	private synchronized void processHandshake() throws IOException {
		if(sslEngine.getHandshakeStatus() == HandshakeStatus.NOT_HANDSHAKING) return; // since this may be called either from a reading or a writing thread and because this method is synchronized it is necessary to double check if we are still handshaking.
		if(!tasks.isEmpty()) {
		...
		if(sslEngine.getHandshakeStatus() == SSLEngineResult.HandshakeStatus.NEED_UNWRAP) {
			if(!isBlocking() || readEngineResult.getStatus() == Status.BUFFER_UNDERFLOW) {
				...
				if(read == -1) throw new IOException("connection closed unexpectedly by peer");
				inCrypt.flip();
			}
			inData.compact();
			unwrap();
	...
	@Override public int read(ByteBuffer dst) throws IOException {
		...
		if(!isHandShakeComplete()) if(isBlocking()) while(!isHandShakeComplete())
			processHandshake();
		else {
			processHandshake();
		...
		if(isBlocking() || readEngineResult.getStatus() == Status.BUFFER_UNDERFLOW) if(socketChannel.read(inCrypt) == -1) return -1;
		inCrypt.flip();
		unwrap();
		
		int transfered = transfereTo(inData, dst);
		if(transfered == 0 && isBlocking()) return read(dst); // "transfered" may be 0 when not enough bytes were received or during rehandshaking
		return transfered;
	}
	...
	private int readRemaining(ByteBuffer dst) throws SSLException {
		if(inData.hasRemaining()) return transfereTo(inData, dst);
	...
	@Override public void close() throws IOException {
		...
		if(socketChannel.isOpen()) socketChannel.write(wrap(emptybuffer));
		socketChannel.close();
		exec.shutdownNow();
	}
	...
	@Override public boolean isNeedWrite() {
		return outCrypt.hasRemaining() || !isHandShakeComplete();
	}
	
	@Override public void writeMore() throws IOException {
		write(outCrypt);
	}
	
	@Override public boolean isNeedRead() {
	...
	@Override public int readMore(ByteBuffer dst) throws SSLException {
