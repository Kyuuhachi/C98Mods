...
import c98.wiiu.websocket.drafts.*;
import c98.wiiu.websocket.drafts.Draft.CloseHandshakeType;
import c98.wiiu.websocket.drafts.Draft.HandshakeState;
...
import c98.wiiu.websocket.server.WebSocketServer.WebSocketWorker;
...
public class WebSocketImpl implements WebSocket {
	...
	public WebSocketImpl(WebSocketListener listener, List<Draft> drafts) {
		...
		if(drafts == null || drafts.isEmpty()) knownDrafts = defaultdraftlist;
		else knownDrafts = drafts;
	}
	...
	public void decode(ByteBuffer socketBuffer) {
		...
		if(DEBUG) System.out.println("process(" + socketBuffer.remaining() + "): {" + (socketBuffer.remaining() > 1000 ? "too big to display" : new String(socketBuffer.array(), socketBuffer.position(), socketBuffer.remaining())) + "}");
	...
	private boolean decodeHandshake(ByteBuffer socketBufferNew) {
		...
		if(tmpHandshakeBytes.capacity() == 0) socketBuffer = socketBufferNew;
		else {
		...
		try {
			...
			try {
				if(role == Role.SERVER) {
					if(draft == null) {
						...
						if(draft == null) close(CloseFrame.PROTOCOL_ERROR, "no draft matches");
						return false;
					} else {
						...
						if(handshakestate == HandshakeState.MATCHED) {
							open(handshake);
							return true;
						} else close(CloseFrame.PROTOCOL_ERROR, "the handshake did finaly not match");
						return false;
					}
					...
					if(handshakestate == HandshakeState.MATCHED) {
						...
						open(handshake);
						return true;
					} else close(CloseFrame.PROTOCOL_ERROR, "draft " + draft + " refuses handshake");
				}
			} catch(InvalidHandshakeException e) {
				close(e);
			}
			...
			if(tmpHandshakeBytes.capacity() == 0) {
				...
				if(newsize == 0) newsize = socketBuffer.capacity() + 16;
		...
		try {
			...
			for(Framedata f : frames) {
				...
				if(curop == Opcode.CLOSING) {
					...
					if(readystate == READYSTATE.CLOSING) // complete the close handshake by disconnecting
					closeConnection(code, reason, true);
					else // echo close handshake
					if(draft.getCloseHandshakeType() == CloseHandshakeType.TWOWAY) close(code, reason, true);
					...
					if(curop != Opcode.CONTINUOUS) {
						if(current_continuous_frame_opcode != null) throw new InvalidDataException(CloseFrame.PROTOCOL_ERROR, "Previous continuous frame sequence not completed.");
						current_continuous_frame_opcode = curop;
					} else if(fin) {
						if(current_continuous_frame_opcode == null) throw new InvalidDataException(CloseFrame.PROTOCOL_ERROR, "Continuous frame sequence was not started.");
						current_continuous_frame_opcode = null;
					} else if(current_continuous_frame_opcode == null) throw new InvalidDataException(CloseFrame.PROTOCOL_ERROR, "Continuous frame sequence was not started.");
					try {
				...
				} else if(current_continuous_frame_opcode != null) throw new InvalidDataException(CloseFrame.PROTOCOL_ERROR, "Continuous frame sequence not completed.");
				else if(curop == Opcode.TEXT) try {
					wsl.onWebsocketMessage(this, Charsetfunctions.stringUtf8(f.getPayloadData()));
				} catch(RuntimeException e) {
					wsl.onWebsocketError(this, e);
				}
				else if(curop == Opcode.BINARY) try {
					wsl.onWebsocketMessage(this, f.getPayloadData());
				} catch(RuntimeException e) {
					wsl.onWebsocketError(this, e);
				}
				else throw new InvalidDataException(CloseFrame.PROTOCOL_ERROR, "non control or continious frame expected");
			}
	...
	private void close(int code, String message, boolean remote) {
		if(readystate != READYSTATE.CLOSING && readystate != READYSTATE.CLOSED) {
			if(readystate == READYSTATE.OPEN) {
				...
				if(draft.getCloseHandshakeType() != CloseHandshakeType.NONE) try {
					if(!remote) try {
			...
			} else flushAndClose(CloseFrame.NEVER_CONNECTED, message, false);
	...
	protected synchronized void closeConnection(int code, String message, boolean remote) {
		...
		if(key != null) // key.attach( null ); //see issue #114
		key.cancel();
		if(channel != null) try {
			channel.close();
		} catch(IOException e) {
	...
	public void closeConnection() {
		if(closedremotely == null) throw new IllegalStateException("this method must be used in conjuction with flushAndClose");
	...
	public void eot() {
		if(getReadyState() == READYSTATE.NOT_YET_CONNECTED) closeConnection(CloseFrame.NEVER_CONNECTED, true);
		else if(flushandclosestate) closeConnection(closecode, closemessage, closedremotely);
		else if(draft.getCloseHandshakeType() == CloseHandshakeType.NONE) closeConnection(CloseFrame.NORMAL, true);
		...
		} else closeConnection(CloseFrame.ABNORMAL_CLOSE, true);
	}
	 ...
	 */
	@Override public void send(String text) throws WebsocketNotConnectedException {
	 ...
	 * @throws NotYetConnectedException
	 */
	@Override public void send(ByteBuffer bytes) throws IllegalArgumentException, WebsocketNotConnectedException {
	...
	@Override public void send(byte[] bytes) throws IllegalArgumentException, WebsocketNotConnectedException {
	...
	private HandshakeState isFlashEdgeCase(ByteBuffer request) throws IncompleteHandshakeException {
		request.mark();
		if(request.limit() > Draft.FLASH_POLICY_REQUEST.length) return HandshakeState.NOT_MATCHED;
		else if(request.limit() < Draft.FLASH_POLICY_REQUEST.length) throw new IncompleteHandshakeException(Draft.FLASH_POLICY_REQUEST.length);
		else {
	...
	private void write(ByteBuffer buf) {
		if(DEBUG) System.out.println("write(" + buf.remaining() + "): {" + (buf.remaining() > 1000 ? "too big to display" : new String(buf.array())) + "}");
	...
	@Override public READYSTATE getReadyState() {
		return readystate;
	}
	...
	@Override public Draft getDraft() {
		return draft;
	}
