package c98.wiiu.websocket.client;
...
import c98.wiiu.websocket.drafts.Draft_17;
...
public abstract class WebSocketClient extends WebSocketAdapter implements Runnable, WebSocket {
	...
	public WebSocketClient(URI serverUri, Draft protocolDraft, Map<String, String> httpHeaders, int connectTimeout) {
		if(serverUri == null) throw new IllegalArgumentException();
		else if(protocolDraft == null) throw new IllegalArgumentException("null as draft is permitted for `WebSocketServer` only!");
		uri = serverUri;
		draft = protocolDraft;
		headers = httpHeaders;
		this.connectTimeout = connectTimeout;
		engine = new WebSocketImpl(this, protocolDraft);
	}
	...
	@Override public void close() {
		if(writeThread != null) engine.close(CloseFrame.NORMAL);
	}
	 ...
	 */
	@Override public void send(String text) throws NotYetConnectedException {
		engine.send(text);
	}
	 ...
	 */
	@Override public void send(byte[] data) throws NotYetConnectedException {
		engine.send(data);
	}
	...
	@Override public void run() {
		try {
			...
			else if(socket.isClosed()) throw new IOException();
			if(!socket.isBound()) socket.connect(new InetSocketAddress(uri.getHost(), getPort()), connectTimeout);
	...
	private int getPort() {
		...
		if(port == -1) {
			...
			if(scheme.equals("wss")) return WebSocket.DEFAULT_WSS_PORT;
	...
	public void setSocket(Socket socket) {
		if(this.socket != null) throw new IllegalStateException("socket has already been set");
		this.socket = socket;
	}
