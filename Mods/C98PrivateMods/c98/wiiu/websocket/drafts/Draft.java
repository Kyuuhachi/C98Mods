...
public abstract class Draft {
	...
	public static HandshakeBuilder translateHandshakeHttp(ByteBuffer buf, Role role) throws InvalidHandshakeException, IncompleteHandshakeException {
		...
		if(firstLineTokens.length != 3) throw new InvalidHandshakeException();
	...
	public List<Framedata> continuousFrame(Opcode op, ByteBuffer buffer, boolean fin) {
		if(op != Opcode.BINARY && op != Opcode.TEXT && op != Opcode.TEXT) throw new IllegalArgumentException("Only Opcode.BINARY or  Opcode.TEXT are allowed");

		if(continuousFrameType != null) continuousFrameType = Opcode.CONTINUOUS;
		else continuousFrameType = op;
		...
		bui.setFin(fin);
		if(fin) continuousFrameType = null;
		else continuousFrameType = op;
	...
	public List<ByteBuffer> createHandshake(Handshakedata handshakedata, Role ownrole, boolean withcontent) {
		...
		if(handshakedata instanceof ClientHandshake) {
			...
			bui.append(" HTTP/1.1");
		} else if(handshakedata instanceof ServerHandshake) bui.append("HTTP/1.1 101 " + ((ServerHandshake)handshakedata).getHttpStatusMessage());
		else throw new RuntimeException("unknow role");
		bui.append("\r\n");
