...
import c98.wiiu.websocket.util.Base64;
...
//	private Framedata fragmentedframe = null;
	...
	@Override public HandshakeState acceptHandshakeAsClient(ClientHandshake request, ServerHandshake response) throws InvalidHandshakeException {
		if(!request.hasFieldValue("Sec-WebSocket-Key") || !response.hasFieldValue("Sec-WebSocket-Accept")) return HandshakeState.NOT_MATCHED;
	...
	@Override public ByteBuffer createBinaryFrame(Framedata framedata) {
		...
		if(sizebytes == 1) buf.put((byte)(payloadlengthbytes[0] | (mask ? (byte)-128 : 0)));
		else if(sizebytes == 2) {
	...
	@Override public List<Framedata> translateFrame(ByteBuffer buffer) throws LimitExedeedException, InvalidDataException {
		List<Framedata> frames = new LinkedList<Framedata>();
		Framedata cur;
		
		if(incompleteframe != null) // complete an incomplete frame
			while(true)
				try {
					buffer.mark();
...
//					int oldsize = incompleteframe.limit();
	...
	public Framedata translateSingleFrame(ByteBuffer buffer) throws IncompleteException, InvalidDataException {
		...
		if(!FIN) if(optcode == Opcode.PING || optcode == Opcode.PONG || optcode == Opcode.CLOSING) throw new InvalidFrameException("control frames may no be fragmented");
		
		if(payloadlength >= 0 && payloadlength <= 125) {} else {
			if(optcode == Opcode.PING || optcode == Opcode.PONG || optcode == Opcode.CLOSING) throw new InvalidFrameException("more than 125 octets");
			if(payloadlength == 126) {
				...
				if(length > Integer.MAX_VALUE) throw new LimitExedeedException("Payloadsize is to big...");
				else payloadlength = (int)length;
			}
		}
		...
		FrameBuilder frame;
		if(optcode == Opcode.CLOSING) frame = new CloseFrameBuilder();
		else {
