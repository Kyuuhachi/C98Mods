...
import java.util.*;
import c98.wiiu.websocket.WebSocket.Role;
...
public class Draft_76 extends Draft_75 {
	...
	private static String generateKey() {
		...
		for(int i = 0; i < numChars; i++) {
			...
			if(randChar >= 48 && randChar <= 57) randChar -= 15;
	...
	private static byte[] getPart(String key) throws InvalidHandshakeException {
		try {
			...
			if(keySpace == 0) throw new InvalidHandshakeException("invalid Sec-WebSocket-Key (/key2/)");
	...
	@Override public HandshakeState acceptHandshakeAsClient(ClientHandshake request, ServerHandshake response) {
		if(failed) return HandshakeState.NOT_MATCHED;
		
		try {
			if(!response.getFieldValue("Sec-WebSocket-Origin").equals(request.getFieldValue("Origin")) || !basicAccept(response)) return HandshakeState.NOT_MATCHED;
			byte[] content = response.getContent();
			if(content == null || content.length == 0) throw new IncompleteHandshakeException();
			if(Arrays.equals(content, createChallenge(request.getFieldValue("Sec-WebSocket-Key1"), request.getFieldValue("Sec-WebSocket-Key2"), request.getContent()))) return HandshakeState.MATCHED;
			else return HandshakeState.NOT_MATCHED;
	...
	@Override public HandshakeState acceptHandshakeAsServer(ClientHandshake handshakedata) {
		if(handshakedata.getFieldValue("Upgrade").equals("WebSocket") && handshakedata.getFieldValue("Connection").contains("Upgrade") && handshakedata.getFieldValue("Sec-WebSocket-Key1").length() > 0 && !handshakedata.getFieldValue("Sec-WebSocket-Key2").isEmpty() && handshakedata.hasFieldValue("Origin")) return HandshakeState.MATCHED;
	...
	@Override public HandshakeBuilder postProcessHandshakeResponseAsServer(ClientHandshake request, ServerHandshakeBuilder response) throws InvalidHandshakeException {
		...
		if(key1 == null || key2 == null || key3 == null || key3.length != 8) throw new InvalidHandshakeException("Bad keys");
		response.setContent(createChallenge(key1, key2, key3));
		return response;
	}
	
	@Override public Handshakedata translateHandshake(ByteBuffer buf) throws InvalidHandshakeException {
	...
	@Override public List<Framedata> translateFrame(ByteBuffer buffer) throws InvalidDataException {
		...
		if(frames == null) {
			...
			frames = readyframes;
			readingState = true;
			if(currentFrame == null) currentFrame = ByteBuffer.allocate(2);
			else throw new InvalidFrameException();
			if(buffer.remaining() > currentFrame.remaining()) throw new InvalidFrameException();
			else currentFrame.put(buffer);
