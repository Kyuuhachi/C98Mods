...
public class CloseFrameBuilder extends FramedataImpl1 implements CloseFrame {
	...
	private void setCodeAndMessage(int code, String m) throws InvalidDataException {
		...
		if(code == CloseFrame.NOCODE) {
			if(0 < m.length()) throw new InvalidDataException(PROTOCOL_ERROR, "A close frame must have a closecode if it has a reason");
	...
	private void initCloseCode() throws InvalidFrameException {
		...
		if(payload.remaining() >= 2) {
			...
			if(code == CloseFrame.ABNORMAL_CLOSE || code == CloseFrame.TLS_ERROR || code == CloseFrame.NOCODE || code > 4999 || code < 1000 || code == 1004) throw new InvalidFrameException("closecode must not be sent over the wire: " + code);
		}
		payload.reset();
	}
	
	@Override public int getCloseCode() {
		return code;
	}
	...
	@Override public void setPayload(ByteBuffer payload) throws InvalidDataException {
		super.setPayload(payload);
		initCloseCode();
		initMessage();
	}
	
	@Override public ByteBuffer getPayloadData() {
