...
import c98.wiiu.websocket.framing.Framedata;
...
public abstract class WebSocketServer extends WebSocketAdapter implements Runnable {
	...
	public WebSocketServer(InetSocketAddress address, int decodercount, List<Draft> drafts, Collection<WebSocket> connectionscontainer) {
		if(address == null || decodercount < 1 || connectionscontainer == null) throw new IllegalArgumentException("address and connectionscontainer must not be null and you need at least 1 decoder");
		...
		synchronized(this) {
			if(selectorthread != null) {
				...
				if(selectorthread != Thread.currentThread()) {
					if(socketsToClose.size() > 0) selectorthread.join(timeout);// isclosed will tell the selectorthread to go down after the last connection was closed
			...
			while(!selectorthread.isInterrupted()) {
				...
				try {
					...
					while(i.hasNext()) {
						...
						if(!key.isValid()) // Object o = key.attachment();
						continue;
							...
							try {
								if(SocketChannelIOHelper.read(buf, conn, conn.channel)) {
									if(buf.hasRemaining()) {
										...
										queue(conn);
										i.remove();
										if(conn.channel instanceof WrappedByteChannel) if(((WrappedByteChannel)conn.channel).isNeedRead()) iqueue.add(conn);
									} else pushBuffer(buf);
								} else pushBuffer(buf);
							} catch(IOException e) {
								pushBuffer(buf);
						...
						if(key.isWritable()) {
							conn = (WebSocketImpl)key.attachment();
							if(SocketChannelIOHelper.batch(conn, conn.channel)) if(key.isValid()) key.interestOps(SelectionKey.OP_READ);
						}
					}
	...
	protected void allocateBuffers(WebSocket c) throws InterruptedException {
		if(queuesize.get() >= 2 * decoders.size() + 1) return;
	...
	private void handleIOException(SelectionKey key, WebSocket conn, IOException ex) {
		...
		if(conn != null) conn.closeConnection(CloseFrame.ABNORMAL_CLOSE, ex.getMessage());
		else if(key != null) {
	...
	@Override @Deprecated public/*final*/void onWebsocketMessageFragment(WebSocket conn, Framedata frame) {// onFragment should be overloaded instead
	...
	@Override public final void onWebsocketOpen(WebSocket conn, Handshakedata handshake) {
		if(addConnection(conn)) onOpen(conn, (ClientHandshake)handshake);
	}

	@Override public final void onWebsocketClose(WebSocket conn, int code, String reason, boolean remote) {
		selector.wakeup();
		try {
			if(removeConnection(conn)) onClose(conn, code, reason, remote);
		} finally {
			try {
				releaseBuffers(conn);
			} catch(InterruptedException e) {
	...
	protected boolean removeConnection(WebSocket ws) {
		boolean removed;
		synchronized(connections) {
			removed = connections.remove(ws);
			assert removed;
		}
		if(isclosed.get() && connections.size() == 0) selectorthread.interrupt();
		return removed;
	}
	...
	protected boolean addConnection(WebSocket ws) {
		if(!isclosed.get()) synchronized(connections) {
			boolean succ = connections.add(ws);
			assert succ;
			return succ;
		}
		else {
	...
	@Override public final void onWriteDemand(WebSocket w) {
	...
	/**
	 * @see WebSocket#sendFragmentedFrame(c98.wiiu.websocket.framing.Framedata.Opcode,
	 *      ByteBuffer, boolean)
	 */
	...
	public interface WebSocketServerFactory extends WebSocketFactory {
		...
		@Override public WebSocketImpl createWebSocket(WebSocketAdapter a, List<Draft> drafts, Socket s);
