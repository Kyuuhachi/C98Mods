...
@ASMer class FoodInfo extends ItemFood {
	...
	@Override public void addInformation(ItemStack is, EntityPlayer ep, List l, boolean adv) {
		...
		if(b) {
			if(isSuperGold) {
				...
				l.add(string(new PotionEffect(MobEffects.FIRE_RESISTANCE, 6000, 0), 1));
	...
	private static String string(PotionEffect e, float chance) {
		String color = (e.potion.isBadEffect() ? TextFormatting.RED : TextFormatting.GREEN).toString();
		String effectS = I18n.format(e.potion.getName());
